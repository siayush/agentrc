...
Build each investigator's prompt from this template; fill in the placeholders. Append the playbook matching this investigator's category: `sources/code-archaeology.md` or `sources/in-repo-records.md`. If the target code looks defensive (null checks, retry logic, timeout handling, rate limiting, feature flags, egress guards, OOM handlers), also append `sources/incident-postmortem.md`.

Tell the source control investigator whether `gh` is authenticated. You already know from `gh auth status`, and without it every PR body and review thread is out of reach, which changes what that investigator can promise.
...
Everything runs locally through git, `gh`, and the filesystem. There is no ticket tracker, wiki, chat, dashboard, error tracker, or warehouse to query, and no credentials exist for any of them. Don't speculate about what those would have contained, and don't present a guess about them as a finding. When the trail leads into one, record the pointer (a ticket ID in a commit message, a doc URL in a PR body) as a lead and say it couldn't be opened.

{SOURCE_PLAYBOOK_SECTION}

### Rules

- **Read-only.** Never commit, never write to the repo, never modify state. Reading historical objects (`git show <hash>^:<path>`) is fine; checking anything out is not.
- **Record commands verbatim.** Your "What I Searched" section should let a reader rerun exactly what you ran and get what you got.
- **Bound broad searches.** Exclude `node_modules`, `vendor/`, and generated output, or a keyword sweep will bury the signal.
- **Dates matter more than usual here.** Almost every judgment in this investigation turns on what came before what. Report the date with every artifact.
...
3. **Follow links within your assigned source.** If a PR references another PR or commit, pull it. If a doc links another doc in the tree, read it. Stay inside your assigned source; when you spot something belonging to the other investigator, record it under "Additional Leads" instead of chasing it. Links that leave the repo entirely (a ticket, a wiki page, a chat thread) can't be followed at all, so log them as leads and move on.
4. **Capture quotes verbatim** with their location (commit hash, PR number, file:line). The synthesizer needs to cite this precisely.
...
### Source
Which source you investigated: source control history, or in-repo written records.
...
### Additional Leads
Two kinds. First, anything for the other investigator: a doc path named in a PR body, a PR number found in a doc's commit history.

Second, and more important here, every pointer into a system nobody can open. Ticket IDs, wiki URLs, chat permalinks, dashboard links, error-tracker issue IDs, anything of that shape mentioned in a commit message, PR body, or file. List each with where you found it. The user can open what you cannot, and this list is often the most actionable part of the investigation.
