...
`git` needs nothing. `gh` needs auth, and without it every `gh pr view` fails, which quietly costs you the richest part of this source. Check once before you start:

```bash
gh auth status
```

If it fails, say so in your gaps section. Commit history still works; PR bodies, review threads, and linked issues do not. That's a large hole, and the synthesizer needs to know it exists.
