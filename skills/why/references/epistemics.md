...
Phrasing: "We searched X, Y, and Z and found no evidence of why." Be specific about *what* you searched. "We couldn't find out" is less useful than "we scanned the 6 PRs that touched this file since 2023, read every commit message on it, and grepped the tree for prose mentioning the threshold; none surfaced a rationale."

One gap recurs in every investigation and deserves its own sentence in the output: only the repository was searched. Rationale that lived in a ticket, a wiki page, or a chat thread and never reached a commit or a committed doc can't be seen from here. Name it as a limit rather than letting silence imply full coverage.
