...
Naming a file isn't grounding. Produce the traced model `how` prescribes. If the design redefines ownership or layering, dig up why the existing shape is what it is before overwriting it. `git log --follow`, `git blame` on the boundary in question, and the PR discussions behind the last few changes. A constraint someone hit two years ago is still a constraint, and rediscovering it during Phase D is expensive.
...
Arena picks the runners. Its default of one each on `opus`, `fable`, and `sonnet` is the right spread here, since design work is judgment-sensitive and the differences between models are the signal.
